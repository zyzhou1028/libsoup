...
soup_uri_to_g_uri (SoupURI *uri)
{
	...
	if (!uri)
        ...
        if (uri->port != 0 && uri->port != soup_scheme_default_port (uri->scheme))
                port = uri->port;

	return g_uri_build_with_user (G_URI_FLAGS_ENCODED | G_URI_FLAGS_HAS_PASSWORD,
				      uri->scheme,
...
soup_uri_new_with_base (SoupURI *base, const char *uri_string)
{
	...
	base_guri = soup_uri_to_g_uri (base);
	guri = g_uri_parse_relative (base_guri, uri_string, G_URI_FLAGS_ENCODED | G_URI_FLAGS_HAS_PASSWORD, NULL);
	if (base_guri)
...
soup_uri_to_string (SoupURI *uri, gboolean just_path_and_query)
{
	...
	g_uri_unref (guri);
        ...
        if (just_path_and_query) {
                /* GUri always adds a : after the scheme which is empty. */
                memmove (ret, ret + 1, strlen (ret) + 1);
        }
